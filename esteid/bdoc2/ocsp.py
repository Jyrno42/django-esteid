...
from asn1crypto.ocsp import OCSPResponse, TBSRequestExtension
...
class OCSP(object):
    ...
    def build_ocsp_request(cls, subject_cert, issuer_cert, signature=None):
        ...
        if signature is not None:
            assert ocsp_request['tbs_request']['request_extensions'][0]['extn_id'].native == 'nonce'
            nonce = cls.build_nonce(signature)
            ocsp_request['tbs_request']['request_extensions'][0] = TBSRequestExtension({
                u'extn_id': u'nonce',
                u'critical': False,
                u'extn_value': nonce.dump(),
            })
        return ocsp_request
